...
class ImagePreprocessor:
    """Tiền xử lý ảnh trước khi đưa vào các nhánh xử lý truyền thống."""
    ...
    def to_grayscale(self, image_bgr: np.ndarray) -> np.ndarray:
        """Chuyển ảnh BGR sang ảnh xám để giảm số kênh xử lý."""
        return cv2.cvtColor(image_bgr, cv2.COLOR_BGR2GRAY)

    def resize_by_width(self, image: np.ndarray) -> np.ndarray:
        """Resize ảnh theo chiều rộng nhưng vẫn giữ đúng tỷ lệ gốc."""
    ...
    def apply_median_filter(self, gray_image: np.ndarray) -> np.ndarray:
        """Khử nhiễu hạt bằng Median Filter nhưng vẫn giữ biên tương đối tốt."""
    ...
    def apply_illumination_correction(self, gray_image: np.ndarray) -> np.ndarray:
        """Cân bằng nền vải bằng Top-Hat hoặc Black-Hat."""
        if not self.apply_tophat:
            return gray_image

        # Kernel lớn dùng để ước lượng nền sáng/tối không đều trên bề mặt vải.
        kernel = cv2.getStructuringElement(
            cv2.MORPH_ELLIPSE,
            (self.tophat_kernel, self.tophat_kernel),
        )

        if self.defect_mode == "both":
            # Kết hợp lỗi sáng và lỗi tối để không phụ thuộc vào loại khuyết tật.
            ...
            # Top-Hat làm nổi các vùng sáng nhỏ hơn nền cục bộ.
            corrected = cv2.morphologyEx(gray_image, cv2.MORPH_TOPHAT, kernel)
        else:
            # Black-Hat làm nổi các vùng tối như vết bẩn hoặc sợi đứt.
    ...
    def apply_statistical_threshold(self, gray_image: np.ndarray) -> tuple[float, np.ndarray]:
        """Tạo mặt nạ nhị phân bằng ngưỡng thống kê Mean ± k*Std."""
        mean, std = cv2.meanStdDev(gray_image)
        mean_value = float(mean[0][0])
        std_value = float(std[0][0])

        # Sau Top-Hat/Black-Hat, vùng lỗi thường sáng hơn nền nên dùng ngưỡng phía trên.
        if self.apply_tophat or self.defect_mode in ("bright", "both"):
            threshold_value = mean_value + self.k_std * std_value
    ...
    def save_histogram(self, output_path: Path, gray_image: np.ndarray, threshold_value: float) -> None:
        """Vẽ histogram mức xám và đánh dấu vị trí ngưỡng cắt."""
        ensure_directory(output_path.parent)

        # Biểu đồ này dùng để giải thích vì sao ảnh được tách thành nền và vùng nghi lỗi.
    ...
    def process(self, image_path: Path, output_dir: Path | None = None) -> dict[str, Any]:
        """Thực thi toàn bộ chuỗi tiền xử lý và lưu ảnh trung gian nếu có output_dir."""
        # Bước 1: đọc ảnh, chuyển xám và chuẩn hóa kích thước.
        image_bgr = self.read_image(image_path)
        gray_image = self.to_grayscale(image_bgr)
        gray_image = self.resize_by_width(gray_image)

        # Bước 2: lọc nhiễu và cân bằng nền trước khi tách ngưỡng.
        ...
        # Lưu các artifact trung gian để đưa vào báo cáo Chương 3.
