...
def save_histogram(output_path: Path, gray_image: np.ndarray, threshold_value: float) -> None:
    """Vẽ histogram mức xám và đánh dấu ngưỡng Otsu."""
    ensure_directory(output_path.parent)

    figure, axis = plt.subplots(figsize=(8, 5))
    axis.hist(gray_image.ravel(), bins=256, range=(0, 255))
    axis.axvline(threshold_value, linestyle="--", linewidth=2, label=f"Otsu T = {threshold_value:.2f}")
    axis.set_title("Histogram mức xám và ngưỡng Otsu")
...
def preprocess_image(
    ...
    if output_dir is not None:
        ...
        save_histogram(output_dir / "01b_histogram.png", median_image, threshold_value)
