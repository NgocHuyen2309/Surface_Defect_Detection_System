...
Tập dữ liệu thực nghiệm được tổ chức theo cấu trúc mỗi lớp nằm trong một thư mục con của `data/raw`. Cấu trúc này giúp các script xử lý có thể tự động đọc ảnh, suy luận nhãn từ tên thư mục và lưu kết quả theo đúng từng lớp.
...
Môi trường xử lý ảnh sử dụng Python, OpenCV và NumPy. Ảnh đầu vào đi qua các bước grayscale, Median Filter, Otsu thresholding và Canny edge detection. Kết quả trung gian được lưu vào `data/processed/canny/` theo từng lớp và từng ảnh. Các file này dùng để kiểm tra trực quan chất lượng xử lý và chọn ảnh minh họa cho báo cáo.
...
Sau khi chạy `scripts/summarize_dataset.py`, số liệu thống kê được lưu trong `docs/dataset_stats/`. Các bảng này có thể đưa trực tiếp vào mục 6.1 để mô tả tổng số ảnh, số lớp, định dạng ảnh và kích thước ảnh.
